...
const KEY = "usegrokbot:submissions";
...
export default function SubmitPage() {
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.localStorage.setItem(KEY, JSON.stringify([...existing, { ...data, at: new Date().toISOString() }]));
    } catch {
      /* local only */
    }
    setSent(true);
    ...
    <div className="mx-auto max-w-[760px] px-5 py-12 md:px-8 md:py-16">
      ...
      {sent ? (
        <div className="mt-10 rounded-[14px] border border-line bg-card p-6">
          <p className="text-ink">{t("submit.thanks")}</p>
        </div>
      ) : (
